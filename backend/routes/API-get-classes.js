...
const LOGOUT = 'https://educ.naghshejahan.ac.ir/logout.aspx';
        ...
        */
        ...
        if (lessons.length > 0) {
            ...
            try {
                ...
                console.warn("Failed to logout:", logoutErr.message);
            }

            res.json(lessons);
        } else {
            res.status(500).json({ error: 'برنامه ای دریافت نشد' });
        }

    } catch (error) {
        console.error("Error fetching schedule:", error.message);
        res.status(500).json({ error: 'مشکل در دریافت برنامه، لحظاتی دیگر تلاش کنید' });
    }
});
